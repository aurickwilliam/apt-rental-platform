...
- **Deferred / blocked:** `get_conversations` hardening and contract additions, chat-list metadata-scan removal, database query-plan/index work, direct Giphy URLs, and any data/schema/policy/generated-type change remain outside the completed client work.
...
- **Blocked RPC metadata work:** deployed `get_conversations(uuid)` is `SECURITY DEFINER`, accepts a caller-supplied internal `public.users.id`, and is executable by `PUBLIC`, `anon`, and `authenticated`. It returns the current conversation fields but not `last_sender_id` or `last_message_type`. Removing client metadata scans before security hardening, RLS validation, contract additions, and old-client compatibility evidence would satisfy `unapprovedOptimization`.
...
4. **Potential Base64 upload overhead — source-observed, not an audit finding.** `useImageUpload` currently requests `base64: true`, calls `decode`, and uploads decoded bytes. No audit baseline, defined correctness property, device memory/transfer measurement, Expo 55 compatibility result, or product threshold establishes that this is harmful enough to change. It is evidence-gathering only.
...
| Conversation-list metadata scans / `get_conversations` | **Blocked — security and deployment** | Deployed RPC is `SECURITY DEFINER`; accepts caller-supplied internal user ID; executable by `PUBLIC`, `anon`, and `authenticated`; lacks `last_sender_id`/`last_message_type`. | Explicit deployment/security approval; approved hardening design; authenticated-only execution; identity from `auth.uid()` via `public.users.id`; RLS/grant/contract tests; nullability and old-client compatibility verification. | Do not remove the scan, add contract fields, alter grants, or generate types until every prerequisite is approved and tested. |
...
| Direct Giphy URLs | **Deferred — product/security/deployment** | Current message interpretation/storage contract remains the compatibility baseline. | Product/security approval, durability confirmation, schema-contract and old-client compatibility evidence. | Do not change message interpretation or delete/rewrite stored assets. |
| Profile Base64 upload | **Evidence-gathering only** | `useImageUpload` uses `base64: true` → decode → upload; apartment edit uses `fetch(uri).blob()` separately. This is not audit M6. | Expo 55 device/release compatibility, controlled transfer and peak-memory benchmark, failure/retry characterization, and a defined success threshold. | No transport change, dependency change, or shared upload abstraction before the gate passes. |
...
This section is a prerequisite specification, **not authorization to execute a migration**.

Only after explicit deployment and security approval may a future design consider a minimal, additive, version-compatible RPC hardening migration. Its contract must be reviewed for these conditions before any client scan is removed:

1. Invocation is restricted to the approved authenticated role; `PUBLIC`/anonymous access is not retained unless explicitly justified and approved.
2. Caller identity is derived from `auth.uid()` and resolved to the internal `public.users.id` inside the approved authorization model; the RPC must not trust a caller-supplied internal user identifier for authorization.
3. `SECURITY DEFINER` behavior, owner, search path, grants, RLS interaction, error semantics, nullability, and tenant/landlord access are tested against authorized and unauthorized accounts.
4. If metadata is added, `last_sender_id` and `last_message_type` are authoritative, explicitly typed/nullability-defined, and additive without breaking result-column order or supported old clients.
5. Contract, RLS, and old-client tests pass before generated types are regenerated and before the client stops performing the existing metadata scan.
6. Rollout retains the existing scan/adapter as a compatible fallback until staged release evidence proves the new contract works for both the new and prior supported mobile versions.

No SQL, grant, policy, type, or client implementation change is part of this design revision.
