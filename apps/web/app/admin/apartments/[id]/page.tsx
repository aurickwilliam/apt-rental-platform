...
export default async function AdminApartmentDetailPage({ params }: PageProps) {
  ...
  const { data: apartment } = await supabase
    .from("apartments")
    .select(
      "id, name, description, street_address, barangay, city, province, status, is_verified, landlord_id, monthly_rent",
    )
    .eq("id", id)
    .is("deleted_at", null)
    .single();
  if (!apartment) notFound();
  const [{ data: landlord }, { data: history }] = await Promise.all([
    supabase
      .from("users")
