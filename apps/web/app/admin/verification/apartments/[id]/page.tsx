...
export default async function ApartmentVerificationReviewPage({
  ...
  const { data: verification } = await supabase
    ...
    .select(
      "id, apartment_id, landlord_id, status, submitted_at, rejection_reason",
    )
    .eq("id", id)
    .single();
  if (!verification) notFound();
  const [{ data: apartment }, { data: landlord }, { data: images }] =
    await Promise.all([
      supabase
        .from("apartments")
        .select(
          "name, street_address, barangay, city, province, monthly_rent, is_verified, lease_agreement_url",
        )
  ...
  const leaseUrl = apartment?.lease_agreement_url
    ? (
        await supabase.storage
          .from("lease-agreements")
          .createSignedUrl(apartment.lease_agreement_url, 60 * 10)
      ).data?.signedUrl
    : null;
  return (
    <div className="mx-auto w-full max-w-5xl space-y-5 p-4">
      <div>
        ...
        <h1 className="mt-3 font-nunito text-3xl font-bold">
      ...
      <section className="rounded-xl border border-border p-4">
        ...
        <dl className="mt-3 grid gap-3 text-sm sm:grid-cols-3">
          <div>
            ...
            <dd>{apartment?.name ?? "Unavailable"}</dd>
          </div>
          <div>
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {images.map((image) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="overflow-hidden rounded-xl border border-border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
              >
                <img
                  src={image.url}
                  alt={`${apartment?.name ?? "Apartment"} listing image`}
                  width={640}
                  height={480}
