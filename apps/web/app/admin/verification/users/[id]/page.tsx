...
export default async function UserVerificationReviewPage({
  params,
}: PageProps) {
  await requireAdmin();
  ...
  const { data: verification } = await supabase
    ...
    .select(
      "id, user_id, id_type, id_front_path, id_back_path, selfie_path, status, submitted_at, rejection_reason",
    )
    .eq("id", id)
    .single();
          ...
          {urls.map((url, index) =>
            url ? (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="rounded-xl border border-border p-3 text-sm font-semibold text-primary hover:bg-muted"
              >
                Open {["ID front", "ID back", "Selfie"][index]}
              </a>
            ) : null,
          )}
        </div>
      ...
      {verification.status === "pending" ? (
        <section className="rounded-xl border border-border p-4">
          ...
          <ReviewForm
            ...
            onReview={reviewUserVerification}
          />
        </section>
      ) : null}
    </div>
