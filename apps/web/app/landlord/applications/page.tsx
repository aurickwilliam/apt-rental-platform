...
export default function ApplicationsPage() {
  ...
  const { applications, loading, error } = useLandlordApplications();
  const [search, setSearch] = useState("");
  const [statusFilter, setStatusFilter] = useState<DisplayStatus | "All">("All");
  const [cityFilter, setCityFilter] = useState<string>("All");

  const cities = useMemo(
    () => [...new Set(applications.map((a) => a.apartment_city).filter(Boolean))].sort(),
    [applications],
  );
  ...
  }, [applications, search, statusFilter, cityFilter]);
  ...
  return (
    <div className="p-4 flex flex-col gap-4 bg-card min-h-0">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">View and manage tenant applications for your properties.</p>
      </div>
                ...
                <Table.ScrollContainer>
                  ...
                  >
                    <Table.Header className="bg-card">
                      <Table.Column isRowHeader className="w-14 text-card-foreground font-medium">Tenant</Table.Column>
                      ...
                      {filtered.map((app) => (
                        <Table.Row key={app.id} id={app.id} className="cursor-pointer hover:bg-muted/50">
                          ...
                          <Table.Cell>
                            ...
                            <p className="text-xs text-muted-foreground">{formatPesoDisplay(app.monthly_rent)}/mo</p>
                          </Table.Cell>
                          <Table.Cell>
                          ...
                          <Table.Cell>
                            <p className="text-sm text-card-foreground">{formatDate(app.created_at, "medium")}</p>
                            <p className="text-xs text-muted-foreground">Move: {formatDate(app.move_in_date, "medium")}</p>
                          </Table.Cell>
                          <Table.Cell>
              ...
              filtered.map((app) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1 text-xs text-muted-foreground">
                    <Calendar size={12} />
                    Submitted {formatDate(app.created_at, "medium")}
                  </div>
                </button>
              ))
            )}
