...
export default function LanguageAndRegion() {
  ...
  const renderSheetHeader = (label: string) => (
    <View>
      <Text className='text-lg text-center text-text font-interMedium border-b border-grey-200 pb-3 mb-4'>
        {label}
      </Text>
    </View>
  )
  ...
  const renderSheetEmpty = () => (
    <View className='h-full py-8 items-center justify-center'>
      <Text className='text-lg text-gray-500 font-inter'>No options available</Text>
    </View>
  )
        ...
        <ListGroup>
          <ListGroup.Item onPress={() => setLanguageSheetOpen(true)}>
            ...
            <ListGroup.ItemSuffix>
              <Text className='text-mediumGrey text-sm font-inter mr-1'>
                {selectedLanguage}
              </Text>
            </ListGroup.ItemSuffix>
          </ListGroup.Item>
          ...
          >
            <BottomSheetFlatList
              ...
              renderItem={({ item }: { item: string }) => (
                <Pressable
                  className={`p-4 rounded-lg mb-2 ${item === selectedLanguage ? 'bg-lightBlue' : 'bg-darkerWhite'}`}
                  onPress={() => {
        ...
        <BottomSheet.Portal>
          ...
          >
            <BottomSheetFlatList
              ...
              renderItem={({ item }: { item: string }) => (
                <Pressable
                  className={`p-4 rounded-lg mb-2 ${item === selectedRegion ? 'bg-lightBlue' : 'bg-darkerWhite'}`}
                  onPress={() => {
